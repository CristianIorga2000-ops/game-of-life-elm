...
var $author$project$Main$GeneratedSeed = function (a) {
	return {$: 'GeneratedSeed', a: a};
};
...
var $author$project$Main$generateRandomSeed = A2($elm$random$Random$generate, $author$project$Main$GeneratedSeed, $elm$random$Random$independentSeed);
var $author$project$Main$SetWindowSize = function (a) {
	return {$: 'SetWindowSize', a: a};
};
var $elm$browser$Browser$Dom$getViewport = _Browser_withWindow(_Browser_getViewport);
var $elm$core$Basics$truncate = _Basics_truncate;
var $author$project$Main$getWindowSize = A2(
	$elm$core$Task$perform,
	function (vp) {
		return $author$project$Main$SetWindowSize(
			_Utils_Tuple2((vp.viewport.width - vp.viewport.x) | 0, (vp.viewport.height - vp.viewport.y) | 0));
	},
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		...
		$elm$core$Platform$Cmd$batch(
			_List_fromArray(
				[$author$project$Main$getWindowSize, $author$project$Main$generateRandomSeed])));
};
...
var $author$project$Main$timeBetweenTransitionsMs = 250;
var $author$project$Main$subscriptions = function (model) {
	var tickCommand = model.isPlaying ? _List_fromArray(
		[
			A2($elm$time$Time$every, $author$project$Main$timeBetweenTransitionsMs, $author$project$Main$Tick)
		]) : _List_Nil;
...
var $author$project$Main$maxGenerated = 1000;
...
var $author$project$Main$cellsGenerator = F2(
	function (maxX, maxY) {
		var generateY = A2($elm$random$Random$int, 0, maxY);
		var generateX = A2($elm$random$Random$int, 0, maxX);
		return A2(
			...
			A2(
				...
				function (cellCount) {
					return A2(
						...
						cellCount,
						A3($elm$random$Random$map2, $elm$core$Tuple$pair, generateX, generateY));
				},
				A2($elm$random$Random$int, 0, $author$project$Main$maxGenerated)));
	});
var $author$project$Rules2D$applyDelta = F2(
	function (_v0, _v1) {
		var cx = _v0.a;
		var cy = _v0.b;
		var dx = _v1.a;
		var dy = _v1.b;
		return _Utils_Tuple2(cx + dx, cy + dy);
	});
var $author$project$Rules2D$deltas = _List_fromArray(
	[
		_Utils_Tuple2(-1, -1),
		_Utils_Tuple2(-1, 0),
		_Utils_Tuple2(-1, 1),
...
var $author$project$Rules2D$game2D = {
	neighboursOf: function (cell) {
		return A2(
			$elm$core$List$map,
			$author$project$Rules2D$applyDelta(cell),
			$author$project$Rules2D$deltas);
	},
	shouldBeBorn: function (x) {
		return x === 3;
	},
	shouldLive: function (x) {
		return (x === 2) || (x === 3);
	}
};
var $author$project$Main$gridSize = 10;
...
var $author$project$Utils$frequencies = function (list) {
	return A2(
		$elm$core$List$map,
		function (_v0) {
			var x = _v0.a;
...
var $author$project$GameOfLife$transition = F2(
	function (gameRules, state) {
		var livingCellsNeighbours = A2($elm$core$List$concatMap, gameRules.neighboursOf, state);
		var freqs = $author$project$Utils$frequencies(livingCellsNeighbours);
		var cellsThatMatch = F2(
			function (neighbourCondition, cells) {
				return A2(
					$elm$core$List$map,
					$elm$core$Tuple$first,
					A2(
						$elm$core$List$filter,
						A2($elm$core$Basics$composeL, neighbourCondition, $elm$core$Tuple$second),
						cells));
			});
		var _v0 = A2(
			$elm$core$List$partition,
			function (x) {
				return A2($elm$core$List$member, x.a, state);
			},
			freqs);
		var freqsOfCells = _v0.a;
		var freqsOfEmptySpaces = _v0.b;
		var survivors = A2(cellsThatMatch, gameRules.shouldLive, freqsOfCells);
		var newborns = A2(cellsThatMatch, gameRules.shouldBeBorn, freqsOfEmptySpaces);
		return _Utils_ap(survivors, newborns);
	});
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'Randomize':
				var maxY = (model.windowHeight / $author$project$Main$gridSize) | 0;
				var maxX = (model.windowWidth / $author$project$Main$gridSize) | 0;
				var _v1 = A2(
					$elm$random$Random$step,
					A2($author$project$Main$cellsGenerator, maxX, maxY),
					model.randomSeed);
				var generatedCells = _v1.a;
				var nextSeed = _v1.b;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{cells: generatedCells, randomSeed: nextSeed}),
					$elm$core$Platform$Cmd$none);
			case 'SetWindowSize':
				var _v2 = msg.a;
				var width = _v2.a;
			...
			case 'CanvasClick':
				var event = msg.a;
				var toCoord = function (f) {
					return (f / $author$project$Main$gridSize) | 0;
				};
				var _v3 = event.pagePos;
				var posX = _v3.a;
				var posY = _v3.b;
				var x = toCoord(posX);
				var y = toCoord(posY);
			...
			case 'Clear':
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 'Tick':
				var newState = A2($author$project$GameOfLife$transition, $author$project$Rules2D$game2D, model.cells);
				var newModel = _Utils_update(
					model,
					{cells: newState});
				return model.isPlaying ? _Utils_Tuple2(newModel, $elm$core$Platform$Cmd$none) : _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			default:
				return _Utils_Tuple2(
					_Utils_update(
						model,
...
var $author$project$Main$buttonHeight = 30;
...
var $author$project$Main$toPos = function (c) {
	return c * $author$project$Main$gridSize;
};
var $author$project$Main$cellsView = function (model) {
	return A2(
		$elm$core$List$map,
		function (cell) {
			return A3($joakin$elm_canvas$Canvas$rect, cell, $author$project$Main$cellSize, $author$project$Main$cellSize);
		},
		A2(
			$elm$core$List$map,
			function (_v0) {
				...
				var y = _v0.b;
				return _Utils_Tuple2(
					$author$project$Main$toPos(x),
					$author$project$Main$toPos(y));
			},
			model.cells));
};
...
var $author$project$Main$renderPlayOrPause = function (model) {
	var buttonMsg = model.isPlaying ? 'Stop' : 'Play';
	return A2(
		...
		_List_fromArray(
			[
				...
				'height',
				$elm$core$String$fromInt($author$project$Main$buttonHeight) + 'px'),
				A2($elm$html$Html$Attributes$style, 'width', '32%')
			]),
		_List_fromArray(
			[
				$elm$html$Html$text(buttonMsg)
			]));
};
...
var $author$project$Main$body = function (model) {
	var truncateByGridSize = function (size) {
		return ((size / $author$project$Main$gridSize) | 0) * $author$project$Main$gridSize;
	};
	var canvasWidth = truncateByGridSize(model.windowWidth - $author$project$Main$borderWidth);
	var canvasHeight = truncateByGridSize(model.windowHeight - $author$project$Main$buttonHeight);
	return _List_fromArray(
		[
			A3(
			...
			_List_fromArray(
				[
					$mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$onClick($author$project$Main$CanvasClick)
				]),
			_List_fromArray(
				[
					A3(
					...
					$author$project$Main$cellsView(model))
				])),
			A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					...
					$author$project$Main$renderPlayOrPause(model)
				]))
		]);
};
